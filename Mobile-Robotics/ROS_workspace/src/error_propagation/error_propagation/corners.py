...
def getCovariance(corner):
	global cov
	global pose

	pointA = corner.p
	pointB = Point(pose.position.x, pose.position.y, pose.position.x)
	dist = getLineBetweenPoints(pointA, pointB)
	Q = numpy.mat('.05 0 0; 0 .05 0; 0 0 .05')
	H = getH(corner, pose, dist.length)

	cov = H * cov * numpy.transpose(H) + Q
	print('Covariance')
	print(cov)
